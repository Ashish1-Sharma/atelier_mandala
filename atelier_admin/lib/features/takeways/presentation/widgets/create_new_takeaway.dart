...
import '../../data/models/takeaway_item_model.dart';

class CreateNewTakeaway extends StatefulWidget {
  final TakeawayModel? model;
  const CreateNewTakeaway({super.key, this.model});
...
class _CreateNewTakeawayState extends State<CreateNewTakeaway> {
          ...
          TextButton(
              onPressed: () async {
                try {
                  ...
                  String? id;
                  // print(downloadUrl);
                  print(widget.model?.tId);
                  try {
                    if (widget.model!.tId == null) {
                  ...
                  // final id = widget.model?.tId == "" ? DateTime.now().millisecondsSinceEpoch.toString() : widget.model?.tId;
                  TakeawayModel model = TakeawayModel(
                      title: title.text,
                  ...
                  AddTakeaway.pushData(model, id!).then((value) => Get.back());
                } catch (e) {
                  print(e);
                }
              },
              ...
              children: [
                ...
                Row(
                  children: [
                    ...
                    Obx(() => Expanded(
                          child: TakeawayController.isProcessing.value
                              ? Container(
                              ...
                              : CustomElevatedButton(
                                  ...
                                  txt: "Publish",
                                  onPressed: () async {
                                    TakeawayController.isProcessing.value =
                                        true;
                                    ...
                                    try {
                                      ...
                                      await AddTakeaway.pushData(model, id!);
                                      Get.back();
                                    } catch (e) {
                                      print(e);
                                      Warnings.onError(
  ...
  Future<void> showImagePickerDialog(BuildContext context) async {
    TakeawayController.imageLinks.clear();
    TakeawayController.isButtonSelected.value = true;
    ...
    return showDialog(
      ...
      builder: (context) => AlertDialog(
            ...
            Row(
              ...
              children: [
                ...
                Expanded(
                  child: TextButton(
                    onPressed: () async {
                      TakeawayController.isButtonSelected.value = false;
                      GlobalController.link.value = '';
                      ...
                      GlobalController.isPickedImageSelected.value =
                          List.generate(
                        TakeawayController.imageLinks.length,
                        (index) => false,
                      );
                    },
                    child: Text(
              ...
              () => Row(
                children: [
                  Expanded(
                    child: Divider(
                      height: 5,
                      color: TakeawayController.isButtonSelected.value
                          ? AppColors.black1
                          : AppColors.black6,
                    ),
                  ),
            ...
            onPressed: () async {
              if(GlobalController.link.value.isNotEmpty){
                ...
                Reference ref = GlobalFirebase.storage
                    .ref()
                    .child("/takeaway/${GlobalController.link.value}");
        ...
        content: Obx(
          () => TakeawayController.isButtonSelected.value
              ? CustomUploadImage()
              : _buildMediaLibrary(),
        ),
      ),
  ...
  Widget _buildMediaLibrary() {
    if (TakeawayController.imageLinks.isEmpty) {
      return Center(
          child:
    ...
    return SingleChildScrollView(
      child: Wrap(
        spacing: 10,
        runSpacing: 10,
        children: List.generate(TakeawayController.imageLinks.length, (index) {
          return InkWell(
            onTap: () {
              ...
              GlobalController.choosenImageLink.value =
                  TakeawayController.imageLinks[index];
            },
            child: Container(
              decoration: BoxDecoration(
                border: Border.all(
  ...
  Future<void> _loadImagesFromFirebase() async {
    try {
      ...
      final listRef = storage
          .ref()
          .child('takeaway'); // Replace 'store' with your folder name
      final result = await listRef.listAll();

      // Ensure imageLinks is updated after all URLs are fetched
      TakeawayController.imageLinks.value = await Future.wait(
...
class PickupTimeSlotsController extends GetxController {
  ...
  List<String> generateTimeSlotsList() {
    return timeSlots.where((slot) {
      final startTime = slot['startTime']?.text.trim();
      final endTime = slot['endTime']?.text.trim();
      return startTime != null && startTime.isNotEmpty && endTime != null && endTime.isNotEmpty;
    }).map((slot) {
